<?php
  ...
  // Let's initialize the tables used, and greate a test user.
	$init = "CREATE TABLE IF NOT EXISTS users (username VARCHAR(254) PRIMARY KEY, password VARCHAR(254), salt VARCHAR(254), type VARCHAR(6) CHECK(type='normal' OR type='admin'))";
     ...
     die("Error in SQL query in initialize.php: " . pg_last_error());
  	...
  	$init = "CREATE TABLE IF NOT EXISTS tasks (id SERIAL PRIMARY KEY, title VARCHAR(40) NOT NULL,
  		...
  		assigner VARCHAR(254) REFERENCES users, owner VARCHAR(254) REFERENCES users NOT NULL,
			...
			status VARCHAR(254) default 'pending' CHECK(status = 'pending' OR status = 'approved' OR status = 'disapproved' OR status = 'completed'))";
  ...
  ('admin', 'b6a7c7264267ceb82adb9e2c84d49edb', '72c2bb70b9062f123e21e34eac743f88', 'admin')";//Mark Password = 1234, Lisa = abcd, admin = password
	...
	if (!$result) {
 	...
 	$sql = "INSERT INTO tasks (title , description , task_date , start_hour , start_min, end_hour, end_min, owner, status) VALUES
	('Second Task' , 'Second task to show.' , '2016-10-04', 8, 30 , 10 , 30 , 'Mark', 'pending'),
	('Third Task' , 'Third task to show.' , '2016-10-05', 9, 30 , 11 , 30 , 'Mark', 'pending'),
	('Water Plants', 'Water the plants near the front desk', '2016-10-15', 9, 00, 9, 15, 'Lisa', 'pending'),
	('Complete TPS Reports', 'Mandatory, needs to be done this Saturday', '2016-10-15', 9, 00, 5, 00, 'Lisa', 'pending')
";
	$result = pg_query($database, $sql);
